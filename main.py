...
CHANNEL_USERNAME = "@SuPeRx1" # قم بتحديث هذا
...
def process_user_link(message):
    ...
    try:
        ...
        # 6. بدء عملية التحميل المباشر لـ تيك توك وإنستجرام (مع التحميل السريع)
        loading_msg = bot.send_message(message.chat.id, f"<strong>⏳ جارٍ التحميل...</strong>", parse_mode="html")
        
        # استدعاء دالة التحميل المحدثة (التي تحاول CDN أولاً)
